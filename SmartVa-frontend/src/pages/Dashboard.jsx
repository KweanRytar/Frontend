...
const Dashboard = () => {
  ...
  const priorityTasks = useMemo(() => {
    const all = [...emergencyTasks, ...overdueTasks, ...pendingTasks, ...dueSoonTasks];
    const unique = Array.from(
      new Map(all.filter((t) => t?._id).map((t) => [t._id, t])).values()
    );
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 dark:text-white pt-20 md:pt-16 pb-12 px-4 sm:px-6 lg:px-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-12">
        ...
        <div className="lg:col-span-2 w-full bg-white dark:bg-gray-800 rounded-2xl p-4 sm:p-6 shadow-sm flex flex-col">
          <h2 className="text-lg font-semibold text-emerald-600 mb-4">Priority Tasks</h2>
        ...
        <div className="w-full bg-white dark:bg-gray-800 rounded-2xl p-4 sm:p-6 shadow-sm flex flex-col">
          <h2 className="text-lg font-semibold text-emerald-600 mb-4">Today's Events</h2>
          <div className="flex flex-col gap-4">
      ...
      <div className="grid lg:grid-cols-2 gap-6 mb-12">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm">
          <h2 className="text-lg font-semibold text-emerald-600 mb-4">Today's Visitors</h2>
          {visitorsToday.length > 0 ? (
            visitorsToday.slice(0, 4).map((visitor) => (
              <Visitors
                ...
                details={() => navigate(`/visitor-details/${visitor._id}`, { state: { visitor } })}
              />
            ))
          ) : (
        ...
        {/* Contacts */}
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm">
          <h2 className="text-lg font-semibold text-emerald-600 mb-4">Recent Contacts</h2>
      ...
      </div>{/* ── end VISITORS + CONTACTS grid ── */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-12">
        ...
        {/* Notes */}
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm">
          <h2 className="text-lg font-semibold text-emerald-600 mb-4">Recently Edited Notes</h2>
        ...
        {/* Documents */}
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm">
          <h2 className="text-lg font-semibold text-emerald-600 mb-4">Recently Updated Documents</h2>
      ...
      </div>{/* ── end NOTES + DOCUMENTS grid ── */}

      {/* ── CALENDAR ── */}
      <div className="w-full bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm">
        <h2 className="text-lg font-semibold text-emerald-600 mb-4">Event Calendar</h2>
      ...
      {showCreateEventModal && (
        <CreateEvent close={() => setShowCreateEventModal(false)} />
      )}
      {showNewTaskModal && (
        <NewTask close={() => setShowNewTaskModal(false)} />
      )}
      {showNewContactModal && (
        <NewContact close={() => setShowNewContactModal(false)} />
      )}
      {showNewDocumentModal && (
        <NewDocument close={() => setShowNewDocumentModal(false)} />
      )}
      {showNewVisitorModal && (
        <NewVisitor close={() => setShowNewVisitorModal(false)} />
      )}
